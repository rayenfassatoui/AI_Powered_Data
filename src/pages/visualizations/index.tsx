...
export default function VisualizationsPage() {
  ...
  const validateMapping = (type: VisualizationType, mapping: Record<string, string>): boolean => {
    switch (type) {
      case 'timeSeries':
  ...
  const updateVisualization = (id: string, updates: Partial<{ type: VisualizationType; mapping: Record<string, string>; config: ChartConfiguration }>) => {
      ...
      prevViz.map(viz => {
        if (viz.id === id) {
          const updatedViz = {
            ...viz,
            ...updates,
            config: updates.config ? { ...defaultChartConfig, ...updates.config } : viz.config
          };
          if (updates.type && !validateMapping(updates.type, updatedViz.mapping)) {
            updatedViz.mapping = {};
          }
          return updatedViz;
        }
    ...
    setChartConfigs(prev => {
      ...
      if (index !== -1) {
        newConfigs[index] = updates.config ? { ...defaultChartConfig, ...updates.config } : visualizations[index].config;
      }
      return newConfigs;
    });
  };
  ...
  const deleteVisualization = (id: string) => {
    ...
    setChartConfigs(prev => prev.filter((_, index) => index !== visualizations.findIndex(v => v.id === id)));
  };
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-4">
            ...
            {visualizations.length > 0 && (
              <Button onClick={() => downloadPDF(visualizations, chartConfigs)} variant="outline" size="lg">
                Export to PDF
              </Button>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 gap-8">
              {visualizations.map((viz) => (
                <Card
                  key={viz.id}
                  className={`${
                    activeVisualization === viz.id ? 'ring-2 ring-blue-500' : ''
                  }`}
